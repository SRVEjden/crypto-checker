		...
		<div className='h-screen'>
			<table className='table w-250'>
				<thead className='sticky top-0 bg-base-100 z-20'>
					<tr>
						...
						<th
							...
							onClick={() => alphabetSort('symbol')}
						>
							Tag ↕
						</th>
						<th className='cursor-pointer' onClick={() => alphabetSort('name')}>
							Name ↕
						</th>
						<th
							className='cursor-pointer'
							onClick={() => numSort('market_cap')}
						>
							Market Cap ↕
						</th>
						<th
							className='cursor-pointer'
							onClick={() => numSort('current_price')}
						>
							Price ↕
						</th>
						<th
							className='cursor-pointer'
							onClick={() => numSort('price_change_24h')}
						>
							24h Change ↕
						</th>
						<th
							className='cursor-pointer'
							onClick={() => numSort('price_change_percentage_24h')}
						>
							24h Change(%) ↕
						</th>
					</tr>
				...
				<tbody className='z-10'>
					{parsedCryptoList
						.filter(coin => searchValidator(coin, cryptoString))
						.map(coin => (
